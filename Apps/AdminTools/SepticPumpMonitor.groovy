 ...
 *   • FIX: CSV logging rewritten to use download-append-upload pattern
 *         (appendHubFile() doesn't exist in Hubitat)
 *   • REMOVED: Notification device support (use virtual switches instead)
 *
 * V 2.2 - 2025-12-04
 *   • FIX: hubFileExists() replaced with try/catch downloadHubFile()
 *   • FIX: currentSwitch → currentValue("switch")
 *   • FIX: baselineMin/baselineMax now calculated properly
 *   • NEW: Gallons pumped estimation (configurable GPM)
 *   • NEW: Rapid cycling detection (X runs in Y minutes)
 *   • NEW: Daily run count alert
 *   • NEW: Time between runs tracking
 *   • NEW: Lifetime runtime hours (pump odometer)
 *   • NEW: Interval anomaly detection
 *
 * V 2.1 - 2025-12-02
 *   • Single yearly CSV file
 *   • Daily scheduled health check
 *   • Wattage buffer capped at 600 readings
 *   • Chart.js median duration trend graph
 *
 * V 2.0 - 2025-12-02
 *   • Initial historical analysis version
 */
...
definition(
    ...
    iconX2Url: "",
    iconX3Url: "",
    importUrl: "https://raw.githubusercontent.com/jrfarrar/hubitat/master/Apps/AdminTools/SepticPumpMonitor.groovy"
)
...
def getStatusDashboardHtml() {
    ...
    if (state.running) {
        ...
        if (daysSince >= xdays) {
            statusColor = "#cc0000"
            statusText = "OVERDUE"
            details += "<br><span style='color:#cc0000;'>⚠️ ${daysSince} days since last run!</span>"
        } else {
...
def getRecentRunsTableHtml() {
    ...
    state.recentRuns.each { run ->
        def flag = run.anomaly ? "<span style='color:#cc0000;'>⚠️</span>" : ""
...
def checkMonthRollover() {
    ...
    if (state.currentMonth != nowMonth) {
        infolog "Month rollover: ${state.currentMonth} → ${nowMonth}"
...
def appButtonHandler(btn) {
    switch (btn) {
        ...
        case "btnResetHistory":
            log.warn "⚠️ RESETTING ALL HISTORICAL DATA"
