...
def pollDeviceList() {
    ...
    def requestParams =
        [
            uri: "${apiUrl()}/v1/devices?access_token=${state.garadgetAccessToken}",
...
def getHandler(resp, data) {
    def garadgetDevices = []
    //def restDevices = resp.data
    restDevices = parseJson(resp.data)  
    //log.info "${restDevices}"
		restDevices.each { garadget ->
            //log.info "EACH: ${garadget}"
        	if (garadget.connected == true)
                //log.info "ID: ${garadget.id}  NAME:${garadget.name} "
                 garadgetDevices << ["${garadget.id}|${garadget.name}":"${garadget.name}"]
            }
	return garadgetDevices.sort()  
}
