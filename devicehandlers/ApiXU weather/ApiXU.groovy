...
def poll()      {
    if (logEnable) log.debug ">>>>> apixu: Executing 'poll', location: $zipCode"
...
//    log.debug "localTime: $localTime | sunriseTime: $sunriseTime | noonTime: $noonTime | sunsetTime: $sunsetTime"
    ...
    if (timeOfDayIsBetween(sunriseTime, noonTime, localTime, tZ))      {
        if (logEnable) log.debug "between sunrise and noon"
        ...
        if (logEnable) log.debug "between noon and sunset"
        ...
        if (logEnable) log.debug "between sunrise and twilight"
        ...
        if (logEnable) log.debug "between sunset and twilight"
        ...
        if (logEnable) log.debug "between non-twilight"
        lux = 5l
        aFCC = false
    }
    ...
    lux = (lux * cCF) as long
    log.info "condition: $cC | condition text: $cCT | condition factor: $cCF | lux: $lux"
