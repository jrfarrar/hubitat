...
def setVersion(){
    ...
    state.version = "1.6.1 - Shelly BLU Distance MQTT Device Handler"   
}
...
void parseBTHomeData(bleData) {
    // Track message receipt for health monitoring (regardless of data changes)
    state.lastMessageReceived = now()
    
    // Reset reconnect attempts on successful message receipt
    state.reconnectAttempts = 0
    
    // Update health status to online since we received a message
    if (device.currentValue("healthStatus") != "online") {
        infolog "Sensor is now reporting - marking online"
        sendEvent(name: "healthStatus", value: "online")
    }
    
    // Log Shelly timestamp for debugging (if present in payload)
    if (bleData.timestamp) {
        def shellyTime = new Date(bleData.timestamp as Long)
        def latencyMs = now() - (bleData.timestamp as Long)
        debuglog "Shelly timestamp: ${shellyTime.format("yyyy-MM-dd HH:mm:ss", location.timeZone)}, latency: ${latencyMs}ms"
    }
